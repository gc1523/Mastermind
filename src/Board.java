...
public class Board {
    ...
    public List<Result> guess(List<Integer> guess){
        rounds++;
        List<Result> results = new ArrayList<>(Collections.nCopies(NUM_SLOTS, Result.INCORRECT));
        ...
        for (int i = 0; i < NUM_SLOTS; i++) {
            if (results.get(i) == Result.CORRECT) continue;

            int g = guess.get(i);
            if (freqMap.getOrDefault(g, 0) > 0) {
                results.set(i, Result.PRESENT);
                freqMap.put(g, freqMap.get(g) - 1);
            }
        }

        Collections.shuffle(results);
        if (results.equals(List.of(Result.CORRECT, Result.CORRECT, Result.CORRECT, Result.CORRECT))) {
